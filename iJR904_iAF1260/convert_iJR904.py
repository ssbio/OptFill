...
input1=open('iJR904.txt','r')

#these are the output file names, 'w' denotes write operation
output1=open('mets_iJR904.txt','w')
output2=open('rxntype_iJR904.txt','w')
output3=open('Sij_iJR904.txt','w')
output4=open('rxns_iJR904.txt','w')
output5=open('ex_rxns_iJR904.txt','w')
output6=open('irrev_rxns_f_iJR904.txt','w')
output9=open('irrev_rxns_b_iJR904.txt','w')
output7=open('rev_rxns_no_ex_iJR904.txt','w')
output8=open('reg_rxns_iJR904.txt','w')
...
for line in alllines1:	#for each line in the input file
	...
	if '->' in line and '<->' not in line:	#if a forward reaction	
		...
		if bool(re.search("dbbiomass0",rxn)):
			objective=reactant
		...
		if bool(re.search('^EX_',rxn)):	#if exchange reaction, not that it is converted to a boolean of a regular expression search
