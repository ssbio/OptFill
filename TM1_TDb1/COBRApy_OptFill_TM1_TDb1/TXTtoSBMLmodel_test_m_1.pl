...
system("python convertWLS_test_model_1.py");

#create the sbml file
open(SBMLFILE, ">test_model_1.xml") or die "could not create/write to sbml file, reason: $!\n";
...
open(METSFILE, "<mets_m_1.txt") or die "could not open metabolites file, reason: $!\n";
...
open(RXNFILE, "<rxns_m_1.txt") or die "could not read the reaction list file, reason: $!\n";
...
open(STOICHFILE, "<Sij_m_1.txt") or die "could not read stoichiometric file, reason: $!\n";
...
#get reaction type
open(RXNTYPEFILE, "<rxntype_m_1.txt") or die "could not open reaction type file, reason: $!\n";
...
printf SBMLFILE "\t<model id=\"tm_1\" name=\"test_model_1\" fbc:strict = \"true\">\n\n";
...
printf SBMLFILE "\t\t\t<parameter id=\"sucrose_uptake\" value=\"-10\" constant=\"true\"/>\n";
...
for(my $f = 0; $f <= $#rxns; $f++) {
	...
	if ($rxn_type{$rxns[$f]} =~ /^-1$/) {
		...
		if($rxns[$f] =~ /R001ex/) {
			...
			#if here is sucrose uptake, so should be limited
			printf SBMLFILE "\t\t\t<reaction id=\"%s\" reversible=\"true\" compartment=\"%s\" fbc:lowerFluxBound=\"sucrose_uptake\" fbc:upperFluxBound=\"irr_b_ub\">\n", $rxns[$f],$rxn_comps{$rxns[$f]};
...
#define objective
#in this case just ue the first reaction as a dummy objective
