...
$include "rxns_m_1.txt"
	...
	eqncounter_3(p)			counter number of unique reversible connecting solutions present
...
$include "rxntype_m_1.txt"
	...
	CPs_soln_r(j_db,p)	stores the set of reactions used to gapfill the model in a given solution iteration stores reversible
...
FILE RESULT /optfill_final_TM1_TDb1.txt/;
PUT RESULT;
RESULT.pw=1000;
...
EQUATIONS
	...
	CPs_const_7(i,j)
	CPs_const_8(i)
	CPs_const_9(biomass_precursors)
	CPs_const_10(i)
	CPs_const_11(j_db)
	CPs_const_12(j_db)
	CPs_const_13(j_db)
...
CPs_const_4(j_m)..					v_2(j_m) =l= vUB(j_m);
CPs_const_5(j_m)..					v_2(j_m) =g= vLB(j_m);
CPs_const_6(i,j)..					epsilon - M * (1-w(i,j)) =l= S(i,j) * v_2(j);
CPs_const_7(i,j)..					M*w(i,j) =g= S(i,j) * v_2(j);
CPs_const_8(i)..					x(i) =l= sum(j, w(i,j));
CPs_const_9(biomass_precursors)..	x(biomass_precursors) =e= 1;
CPs_const_10(i)..					sum(j, S(i,j) * v_2(j)) =e= 0;
CPs_const_11(j_db)..				delta(j_db) + rho(j_db) - omega(j_db) =e= zeta(j_db);
CPs_const_12(j_db)..				omega(j_db) =l= delta(j_db);
CPs_const_13(j_db)..				omega(j_db) =l= rho(j_db);
CPs_const_14(j_db)..				zeta(j_db) =l= sum(i,w(i,j_db));
...
int_cut_3(eqncounter_3)..			sum(j_db$(CPs_soln_r(j_db,eqncounter_3) eq 1), omega(j_db)) =l= sum(j_db, CPs_soln_r(j_db,eqncounter_3)) - 1;
int_cut_4(eqncounter_2)..			sum(j_db$(CPs_soln_f(j_db,eqncounter_2) eq 1), CPs_soln_f(j_db,eqncounter_2) - delta(j_db)) + sum(j_db$(CPs_soln_b(j_db,eqncounter_2) eq 1), CPs_soln_b(j_db,eqncounter_2) - rho(j_db)) =g= num_omegas(eqncounter_2) + 1;
int_cut_5(eqncounter)..				sum(j_db$(TFP_soln_f(j_db,eqncounter) eq 1), delta(j_db)) =l= sum(j_db, TFP_soln_f(j_db,eqncounter)) - tau(eqncounter);
int_cut_6(eqncounter)..				sum(j_db$(TFP_soln_b(j_db,eqncounter) eq 1), rho(j_db)) =l= sum(j_db, TFP_soln_b(j_db,eqncounter)) - (1 - tau(eqncounter));
...
LOOP(p$(not done),
	...
	IF((CP1.modelstat eq 1),
		...
		PUT "RXN TO ADD  DIRECTION DELTA       RHO         OMEGA        ZETA"/;
		PUT "---------------------------------------------------------------"/;
		...
		LOOP(j_db,
			...
			IF(((omega.l(j_db) le (1 + epsilon)) AND (omega.l(j_db) ge (1 - epsilon))),
				...
				PUT " <-> ",delta.l(j_db),rho.l(j_db),omega.l(j_db),zeta.l(j_db)/;
			...
			ELSE
				...
				PUT " XX  ",delta.l(j_db),rho.l(j_db),omega.l(j_db),zeta.l(j_db)/;
		...
		IF((temp ne 0), 
			...
			eqncounter_3(p)=yes;
			CPs_soln_r(j_db,eqncounter_3(p))=omega.l(j_db);
